#!/bin/bash
# Live-Watch für 2h-30s-Real-Test. Auto-Refresh alle 10s.
CSV=/tmp/obs_2h_30s_after_recovery_real.csv
HB=/tmp/obs_2h_30s_after_recovery_real_heartbeat.txt
DONE=/tmp/obs_2h_30s_after_recovery_real.done
STOPLOG=/tmp/obs_2h_30s_after_recovery_real_stop.log
RESERVE_LB=4.2661642227906995

while :; do
  clear
  echo "NEXUS LIVE WATCH · 30s-REAL · $(date '+%d.%m.%Y %H:%M:%S')"
  echo "════════════════════════════════════════════════════════"
  ...
  LAST=$(tail -1 "$CSV")
  IFS=',' read -r TS UNIX CPU MEM DBMB QC RES GUARD TESTONLY ISLIVE CYCLE DECT DEC1M MALF INC LEAK CANDLE SYSLOG BAL SUBRED AMPEL <<< "$LAST"
  ...
  echo "─── BRAIN / ROUTER ───"
  echo "Decisions total: $DECT   (letzte 60s: $DEC1M)"
