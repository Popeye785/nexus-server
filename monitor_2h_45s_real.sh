...
APIPORT=3000
BASE=/tmp/obs_2h_45s_after_30s_fail_real
CSV=${BASE}.csv
HB=${BASE}_heartbeat.txt
STOPLOG=${BASE}_stop.log
DONE=${BASE}.done
SNAPDIR=$(cat /tmp/snap_pre45s_dir.txt 2>/dev/null | grep -o "/.*" | head -1)
...
trigger_stop() {
  ...
  echo "STOPPED:$1" > "$DONE"
  {
    ...
    echo "  2. cp -p $SNAPDIR/nexus_pre45s_real.db $MAINDB"
...
while :; do
  ...
  [ $ELAPSED -ge $DURATION ] && { echo "DONE:2h_45s_real_clean $(date '+%F %T')" > "$DONE"; exit 0; }
