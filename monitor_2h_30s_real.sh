#!/bin/bash
# 2h-Monitor für 30s-Real-Cycle nach Recovery.
# KORREKTUREN vs Christian-Original:
#   1. TESTONLY aus /api/test-only/status.active (statt /api/mode.testOnly — existiert nicht)
#   2. ISLIVE aus /api/mode.isLive (camelCase, korrekt)
#   3. Reserve aus data/demo_wallet.json via python3 (statt grep)
#   4. SNAPDIR aus tmp-file gelesen
...
APIPORT=3000
BASE=/tmp/obs_2h_30s_after_recovery_real
CSV=${BASE}.csv
HB=${BASE}_heartbeat.txt
STOPLOG=${BASE}_stop.log
DONE=${BASE}.done
SNAPDIR=$(cat /tmp/snap_pre30s_dir.txt 2>/dev/null | grep -o "/.*" | head -1)
...
[ -f "$CSV" ] || echo "ts,unix,cpu,mem_mb,db_mb,db_qc,reserve,guardian,test_only,is_live,cycle_ms,dec_total,dec_1m,malformed,incidents,leak,candle,syslog,balance,subsys_red,ampel" > "$CSV"
...
trigger_stop() {
  ...
  echo "STOPPED:$1" > "$DONE"
  {
    ...
    echo "  2. cp -p $SNAPDIR/nexus_pre30s_real.db $MAINDB"
...
while :; do
  ...
  [ $ELAPSED -ge $DURATION ] && { echo "DONE:2h_30s_real_clean $(date '+%F %T')" > "$DONE"; exit 0; }
  ...
  # KORRIGIERTE Endpoints: TESTONLY hat eigenen Endpoint
  ...
  if [ "$DEC1M" = "0" ]; then ZERO_DEC_STREAK=$((ZERO_DEC_STREAK+1)); else ZERO_DEC_STREAK=0; fi
  [ $ZERO_DEC_STREAK -ge 3 ] && { R=2; RED="decisions=0 x${ZERO_DEC_STREAK}"; }

  case $R in 2) AMPEL="ROT";; 1) AMPEL="GELB";; *) AMPEL="GRUEN";; esac

  echo "$TS,$NOW,$CPU,$MEM,$DB_MB,\"$QC\",$RES,$GUARD,$TESTONLY,$ISLIVE,$CYCLE,$DEC,$DEC1M,$MALF,$INCC,$LEAKC,$CANDLE,$SYSLOG,$BAL,\"${RED:-${RY:-none}}\",$AMPEL" >> "$CSV"
  echo "$(date '+%F %T') ampel=$AMPEL remaining=$(( (DURATION-ELAPSED)/60 ))min dec1m=$DEC1M malf=$MALF qc=$QC reserve=$RES test_only=$TESTONLY live=$ISLIVE cycle=$CYCLE mem=${MEM}MB db=${DB_MB}MB" > "$HB"
