...
BASE=/tmp/obs_60s_canary_2h
CSV=${BASE}.csv; HB=${BASE}_heartbeat.txt; STOPLOG=${BASE}_stop.log; DONE=${BASE}.done
SNAPDIR="/Users/christianheilig/NEXUS_SNAPSHOT_PRE_60s_CANARY_P3_20260602_122429"
RESERVE_LB=4.2661642227906995
DURATION=$((2*3600)); INTERVAL=30; START=$(date +%s)
...
[ -f "$CSV" ] || echo "ts,unix,cpu,mem_mb,db_mb,db_qc,qc_rc,reserve,guardian,test_only,is_live,cycle_ms,dec_total,dec_int,malformed,locked,db_safe,cc_fail,opp_hint,incidents,leak,candle,syslog,opp_log,bal,wal_mb,subsys_red,ampel" > "$CSV"
...
auto_restore(){
  ...
  FORENSIC="$HOME/NEXUS_CANARY_FORENSIC_$(date +%Y%m%d_%H%M%S)"
  mkdir -p "$FORENSIC"
...
while :; do
  ...
  [ $ELAPSED -ge $DURATION ] && { echo "DONE:2h_60s_canary_clean $(date '+%F %T')" > "$DONE"; exit 0; }
  TS=$(date '+%F %T')
  ...
  [ "$CYCLE" != "60000" ] && [ -n "$CYCLE" ] && { R=2; RED="Cycle!=60000 ($CYCLE)"; }
  ...
  echo "$TS,$NOW,$CPU,$MEM,$DB_MB,\"$QC\",$QC_RC,$RES,$GUARD,$TESTONLY,$ISLIVE,$CYCLE,$DEC,$DEC_INT,$MALF,$LOCKED,$DBSAFE,$CCF,$OPPH,$INCC,$LEAKC,$CANDLE,$SYSLOG,$OPPLOG,$BAL,$WAL_MB,\"${RED:-none}\",$AMPEL" >> "$CSV"
  echo "$(date '+%F %T') ampel=$AMPEL rem=$(( (DURATION-ELAPSED)/60 ))min qc=$QC(rc=$QC_RC) malf=$MALF locked=$LOCKED cc_fail=$CCF opp=$OPPH dec_int=$DEC_INT reserve=$RES cycle=$CYCLE mem=${MEM}MB wal=${WAL_MB}MB opp_log=$OPPLOG" > "$HB"
