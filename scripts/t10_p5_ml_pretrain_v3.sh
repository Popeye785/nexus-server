...
LOG="/tmp/t10_p5_v3_${TS}.log"
ML_BACKUP="/tmp/t10_p5_v3_ml_backup_${TS}.sql"
ROLLBACK_DONE=0
SCRIPT_FAILED=0
...
echo "🧠 T10-P5 v3 ML-PRETRAINING (tickLevel=false) 5J GESTARTET" | tee "$LOG"
echo "═══════════════════════════════════════════════════════" | tee -a "$LOG"
...
for COIN in "${COINS[@]}"; do
  ...
  RESULT=$(curl -s -X POST http://localhost:3000/api/training/run \
    -H "Content-Type: application/json" \
    -d "{\"symbol\":\"$COIN\",\"granularity\":\"1h\",\"limit\":$LIM,\"strategy\":\"ema_cross\",\"capital\":1000,\"posSize\":0.1,\"slPct\":0.02,\"tpPct\":0.04,\"tickLevel\":false}" \
