...
APIPORT=3000
BASE=/tmp/obs_8h_60s_overnight
CSV=${BASE}.csv
HB=${BASE}_heartbeat.txt
STOPLOG=${BASE}_stop.log
DONE=${BASE}.done
SNAPDIR=$(ls -dt /Users/christianheilig/NEXUS_SNAPSHOT_PRE_OVERNIGHT_60s_* 2>/dev/null | head -1)
...
trigger_stop() {
  ...
  echo "STOPPED:$1" > "$DONE"
  echo "ROLLBACK: pm2 stop nexus; cp -p $SNAPDIR/nexus_pre_overnight.db $MAINDB; rm -f ${MAINDB}-wal ${MAINDB}-shm; sqlite3 $MAINDB \"UPDATE bot_settings SET value='120000' WHERE key='aladdin_cycle_ms';\"; pm2 start nexus" >> "$STOPLOG"
  exit 2
}

while :; do
  NOW=$(date +%s); ELAPSED=$((NOW-START))
  [ $ELAPSED -ge $DURATION ] && { echo "DONE:8h_60s_overnight_clean $(date '+%F %T')" > "$DONE"; exit 0; }
  TS=$(date '+%F %T')
  ...
  if [ -n "$MEM_BASELINE" ] && [ "$MEM_BASELINE" -gt 0 ] && [ -n "$MEM" ]; then
    ...
    elif [ "$MEM" -gt $((MEM_BASELINE*3)) ]; then [ $R -lt 1 ] && R=1; RY="Mem hoch"; fi
  fi
  [ -n "$DB_BASELINE_MB" ] && [ "$DB_BASELINE_MB" -gt 0 ] && [ -n "$DB_MB" ] && [ "$DB_MB" -gt $((DB_BASELINE_MB*2)) ] && { R=2; RED="DB-Spike"; }
  ...
  echo "$(date '+%F %T') ampel=$AMPEL remaining=$(( (DURATION-ELAPSED)/3600 ))h$(( ((DURATION-ELAPSED)%3600)/60 ))m dec_int=$DEC_INT malf=$MALF qc=$QC reserve=$RES test_only=$TESTONLY live=$ISLIVE cycle=$CYCLE mem=${MEM}MB db=${DB_MB}MB" > "$HB"
